import React, { FC, useEffect, useState } from 'react';
import { EDITOR_TYPES, IViewInputControl, SearchHeaderV3, useSearchHeader } from '@wings-shared/form-controls';
import { agGridUtilities, CustomAgGridReact, useAgGrid, useGridState } from '@wings-shared/custom-ag-grid';
import { useConfirmDialog, useUnsubscribe } from '@wings-shared/hooks';
import {
  DATE_FORMAT,
  GRID_ACTIONS,
  GridPagination,
  IAPIGridRequest,
  IClasses,
  UIStore,
  Utilities,
} from '@wings-shared/core';
import { ColDef, GridOptions, RowNode } from 'ag-grid-community';
import { Grid } from '@material-ui/core';
import { finalize, takeUntil } from 'rxjs/operators';
import { AlertStore } from '@uvgo-shared/alert';
import { ModalStore } from '@uvgo-shared/modal-keeper';
import { useNavigate } from 'react-router-dom';
import { SidebarStore } from '@wings-shared/layout';
import { inject, observer } from 'mobx-react';
import { Dialog } from '@uvgo-shared/dialog';
import { useStyles } from '../PermitReview.styles';
import {
  BaseCountryStore,
  STAGING_REVIEW_COMPARISION_TYPE,
  STAGING_REVIEW_STATUS,
  StagingFieldsRenderer,
  UplinkRejectionNotes,
  UplinkReviewActions,
  VIEW_MODE,
} from '@wings/shared';
import {
  comparisonType,
  fields,
  getPermitInfoReview,
  isDataMerged,
  isDataRejected,
  mergeStatus,
  mergeStatusOptions,
} from '../fields';
import {
  PERMIT_INFO_FILTERS,
  PermitInfoReviewModel,
  PermitSettingsStore,
  PermitStore,
  sidebarOptions,
  updatePermitSidebarOptions,
  usePermitModuleSecurity,
} from '../../Shared';

interface Props {
  permitStore?: PermitStore;
  permitSettingsStore?: PermitSettingsStore;
  sidebarStore?: typeof SidebarStore;
}

const PermitInfoReview: FC<Props> = ({ sidebarStore, permitStore, permitSettingsStore }) => {
  const classes = useStyles();
  const gridState = useGridState();
  const unsubscribe = useUnsubscribe();
  const navigate = useNavigate();
  const agGrid = useAgGrid<PERMIT_INFO_FILTERS, PermitInfoReviewModel>([], gridState);
  const [ entityOptions, setEntityOptions ] = useState([]);
  const _useConfirmDialog = useConfirmDialog();
  const searchHeader = useSearchHeader();
  const _sidebarStore = sidebarStore as typeof SidebarStore;
  const permitModuleSecurity = usePermitModuleSecurity();
  const _permitSettingsStore = permitSettingsStore as PermitSettingsStore;
  const _permitStore = permitStore as PermitStore;

  const baseCountryStore = new BaseCountryStore();

  useEffect(() => {
    sidebarStore?.setNavLinks(sidebarOptions(false), 'permits');
    _permitSettingsStore.loadRejectionReason().subscribe();
    loadInitialData();
  }, []);

  useEffect(() => {
    loadEntityOptions();
  }, [ searchHeader.searchType, searchHeader.searchValue ]);

  /* istanbul ignore next */
  const getFilterPropertyName = (selectedOption): any => {
    switch (selectedOption) {
      case PERMIT_INFO_FILTERS.COUNTRY:
        return 'countryId';
      case PERMIT_INFO_FILTERS.APPROVAL_STATUS:
        return 'mergeStatusId';
    }
  };

  /* istanbul ignore next */
  const filterCollection = (): IAPIGridRequest => {
    const { searchType, chipValue } = searchHeader;
    const _searchValue = chipValue.map(x => {
      return Utilities.isEqual(searchType, PERMIT_INFO_FILTERS.APPROVAL_STATUS) ? x.value : x.id;
    });
    if (!_searchValue || _searchValue.length === 0) {
      return {};
    }
    return {
      filterCollection: JSON.stringify([{ [getFilterPropertyName(searchType)]: _searchValue[0] }]),
    };
  };

  /* istanbul ignore next */
  const loadInitialData = (pageRequest?: IAPIGridRequest) => {
    const request: IAPIGridRequest = {
      pageSize: gridState.pagination.pageSize,
      ...pageRequest,
      ...filterCollection(),
      ...agGrid.filtersApi.gridSortFilters(),
    };
    UIStore.setPageLoader(true);
    _permitStore
      .getPermitInfoReview(request)
      .pipe(
        takeUntil(unsubscribe.destroy$),
        finalize(() => UIStore.setPageLoader(false))
      )
      .subscribe(tableData => {
        const transformedData = getPermitInfoReview(tableData.results, [ 1 ], false);
        gridState.setGridData(transformedData);
        gridState.setPagination(new GridPagination({ ...tableData }));
        const selectAll = gridState.data.some(x => !Utilities.isEqual(x.faaMergeStatus, STAGING_REVIEW_STATUS.MERGED));
        gridState.setAllowSelectAll(selectAll);
      });
  };

  /* istanbul ignore next */
  const loadEntityOptions = () => {
    let observableOf;

    switch (searchHeader.searchType) {
      case PERMIT_INFO_FILTERS.APPROVAL_STATUS:
        setEntityOptions(mergeStatusOptions as any);
        break;
      case PERMIT_INFO_FILTERS.COUNTRY:
        const _searchValue = searchHeader.searchValue;
        if (!_searchValue) {
          setEntityOptions([]);
          return;
        }
        observableOf = baseCountryStore.searchCountries(_searchValue);
        break;
      default:
        break;
    }
    if (observableOf) {
      UIStore.setPageLoader(true);
      observableOf
        .pipe(
          takeUntil(unsubscribe.destroy$),
          finalize(() => UIStore.setPageLoader(false))
        )
        .subscribe(response => setEntityOptions(response));
    }
  };

  const customErrorMessage = (error, id) => {
    if (error.message.toLowerCase().includes(`UplinkProperty ${id} Id not found`.toLowerCase())) {
      AlertStore.critical('Updates have been made to this record. Please refresh the page.');
      return;
    }
    AlertStore.critical(error.message);
  };

  const dialogContent = (data = {}) => {
    const items = Array.isArray(data.results) ? data.results : [];

    // Map property names to specific formatters that extract the nested display name
    const formatters = {
      'PermitAdditionalInfo.AppliedPermitPrerequisiteTypes': item => item?.permitPrerequisiteType?.name ?? '',
      'PermitAdditionalInfo.AppliedBlanketValidityTypes': item => item?.blanketValidityType?.name ?? '',
      'PermitAdditionalInfo.PermitAdditionalInfoRevisions': item => item?.missionElement?.name ?? '',
      PermitRequiredElements: item => item?.element?.name ?? '',
    };

    const firstValidItem = items.find(item => {
      const oldValue = item.oldValue ?? [];
      const newValue = item.newValue ?? [];

      const isOldValid = Array.isArray(oldValue) ? oldValue.length > 0 : !!oldValue;
      const isNewValid = Array.isArray(newValue) ? newValue.length > 0 : !!newValue;

      return isOldValid || isNewValid;
    });

    if (!firstValidItem) return null;

    const { oldValue = [], newValue = [], propertyName } = firstValidItem;
    const formatFn = formatters[propertyName] || (item => item?.name ?? '');

    return (
      <Grid container>
        <Grid item xs={12}>
          <StagingFieldsRenderer
            oldValue={Array.isArray(oldValue) ? oldValue : [ oldValue ]}
            newValue={Array.isArray(newValue) ? newValue : [ newValue ]}
            title={propertyName}
            formatFn={formatFn}
          />
        </Grid>
      </Grid>
    );
  };

  const getApprovalsData = (data: PermitInfoReviewModel) => {
    const simpleNavigationKeys = [ 'LeadTimes', 'PermitValidities', 'PermitDocuments' ];
    // Navigate directly for certain keys
    if (simpleNavigationKeys.includes(data.propertyName)) {
      if (!data?.permitStagingPropertyId) return;
      navigate(`/permits/permit-info-review/${data.permitStagingPropertyId}/${data.id}/review-details`);
      return;
    }

    UIStore.setPageLoader(true);
    _permitStore
      ?.getPermitReviewListDetail(data.permitStagingPropertyId)
      .pipe(
        takeUntil(unsubscribe.destroy$),
        finalize(() => {
          UIStore.setPageLoader(false);
        })
      )
      .subscribe({
        next: response => {
          ModalStore.open(
            <Dialog
              title="Approval Details"
              open={true}
              onClose={() => ModalStore.close()}
              dialogContent={() => dialogContent(response)}
              disableBackdropClick={true}
            />
          );
        },
        error: error => {
          customErrorMessage(error, data.id);
        },
      });
  };

  const confirmApprove = (data: PermitInfoReviewModel): void => {
    _useConfirmDialog.confirmAction(
      () => {
        approveRecord(data), ModalStore.close();
      },
      {
        title: 'Confirm Action',
        message: 'Are you sure you want to approve the changes?',
      }
    );
  };

  const upsertRejectionReason = (rejection, data) => {
    UIStore.setPageLoader(true);
    const request = {
      uplinkStagingId: data?.id,
      rejectionReasonId: rejection.rejectionReason?.id,
      rejectionNotes: rejection.rejectionNotes,
    };
    _permitStore
      ?.rejectPermitInfoStaging(request)
      .pipe(
        takeUntil(unsubscribe.destroy$),
        finalize(() => UIStore.setPageLoader(false))
      )
      .subscribe({
        next: response => {
          if (response.hasErrors) {
            return;
          }
          ModalStore.close();
          gridState.gridApi.deselectAll();
          loadInitialData();
        },
        error: error => {
          ModalStore.close();
          customErrorMessage(error, data.id);
        },
      });
  };

  const openRejectionNotesDialog = (viewMode: VIEW_MODE, data): void => {
    const inputControls: IViewInputControl[] = [
      {
        fieldKey: 'rejectionReason',
        type: EDITOR_TYPES.DROPDOWN,
        isFullFlex: true,
        options: _permitSettingsStore.rejectionReason || [],
      },
      {
        fieldKey: 'rejectionNotes',
        type: EDITOR_TYPES.TEXT_FIELD,
        multiline: true,
        rows: 5,
        isFullFlex: true,
      },
    ];

    ModalStore.open(
      <UplinkRejectionNotes
        viewMode={VIEW_MODE.EDIT}
        fields={fields}
        inputControls={inputControls}
        onDataSave={onSave => upsertRejectionReason(onSave, data)}
      />
    );
  };

  const approveRecord = (data): void => {
    UIStore.setPageLoader(true);
    const request = {
      uplinkStagingId: data?.id,
    };
    _permitStore
      ?.approvePermitInfoStaging(request)
      .pipe(
        takeUntil(unsubscribe.destroy$),
        finalize(() => UIStore.setPageLoader(false))
      )
      .subscribe({
        next: response => {
          if (response.hasErrors) {
            return;
          }
          gridState.gridApi.deselectAll();
          loadInitialData();
        },
        error: error => {
          customErrorMessage(error, data.id);
        },
      });
  };

  const isDataDisabled = data => {
    if (isDataMerged(data.mergeStatus) || isDataRejected(data.mergeStatus)) {
      return true;
    }
    if (
      (Utilities.isEqual(data.comparisionType, STAGING_REVIEW_COMPARISION_TYPE.ADDED) && data.vendorPermitId) ||
      Utilities.isEqual(data.comparisionType, STAGING_REVIEW_COMPARISION_TYPE.MODIFIED)
    ) {
      return false;
    }
    return true;
  };

  const reviewActions = (rowIndex: number, { data }: RowNode) => {
    const isDisabled = isDataDisabled(data);
    return (
      <UplinkReviewActions
        approveRejectPermission={permitModuleSecurity.isEditable && data.vendorPermitId}
        viewDetailsPermission={permitModuleSecurity.isEditable && data.isList}
        disabledApproveReject={isDisabled}
        onApprove={() => confirmApprove(data)}
        onReject={() => openRejectionNotesDialog(VIEW_MODE.EDIT, data)}
        onViewDetails={() => getApprovalsData(data)}
      />
    );
  };

  const columnDefs: ColDef[] = [
    {
      headerName: 'Permit Type',
      field: 'permitType',
      minWidth: 100,
      valueFormatter: ({ value }) => value?.name || '',
      headerTooltip: 'Permit Type',
    },
    {
      headerName: 'Property Name',
      field: 'propertyName',
      minWidth: 150,
      headerTooltip: 'Property Name',
    },
    {
      headerName: 'Old Value',
      field: 'oldValue',
      minWidth: 150,
      headerTooltip: 'Old Value',
    },
    {
      headerName: 'New Value',
      field: 'newValue',
      minWidth: 150,
      headerTooltip: 'New Value',
    },
    {
      headerName: 'Comparison Type',
      field: 'comparisionType',
      minWidth: 120,
      headerTooltip: 'Comparison Type',
      valueFormatter: ({ data }) => {
        return data?.vendorPermitId ? comparisonType[data?.comparisionType] : '';
      },
    },
    {
      headerName: 'Status',
      field: 'mergeStatus',
      minWidth: 120,
      editable: false,
      cellRenderer: 'statusRenderer',
      cellRendererParams: ({ data }) => {
        return {
          value: mergeStatus[data?.mergeStatus],
        };
      },
      headerTooltip: 'Status',
    },
    ...agGrid.auditFields(gridState.isRowEditing),
    {
      ...agGrid.actionColumn({
        cellRenderer: 'viewRenderer',
        maxWidth: 150,
        minWidth: 150,
        cellRendererParams: {
          getViewRenderer: (rowIndex: number, node: RowNode, classes: IClasses) => reviewActions(rowIndex, node),
        },
      }),
    },
  ];

  /* istanbul ignore next */
  const gridOptions = (): GridOptions => {
    const baseOptions: Partial<GridOptions> = agGrid.gridOptionsBase({
      context: {},
      columnDefs,
      isEditable: permitModuleSecurity.isEditable,
    });

    return {
      ...baseOptions,
      defaultColDef: {
        ...baseOptions.defaultColDef,
        suppressMovable: true,
      },
      pagination: false,
      suppressScrollOnNewData: true,
      isExternalFilterPresent: () => false,
      suppressCellSelection: true,
      suppressClickEdit: true,
      suppressRowHoverHighlight: true,
      onFilterChanged: () => loadInitialData({ pageNumber: 1 }),
      onSortChanged: e => {
        agGrid.filtersApi.onSortChanged(e);
        loadInitialData({ pageNumber: 1 });
      },

      groupDefaultExpanded: 0, // Expand all groups by default
      getDataPath: data => {
        return data.path;
      },
      treeData: true,
      autoGroupColumnDef: {
        headerName: 'Country',
        field: 'country',
        minWidth: 180,
        cellRenderer: 'agGroupCellRenderer',
        cellRendererParams: {
          suppressCount: true,
        },
        valueFormatter: ({ data }) => {
          return data?.country.label || '';
        },
      },
    };
  };

  return (
    <>
      <SearchHeaderV3
        useSearchHeader={searchHeader}
        onExpandCollapse={agGrid.autoSizeColumns}
        selectInputs={[
          agGridUtilities.createSelectOption(PERMIT_INFO_FILTERS, PERMIT_INFO_FILTERS.COUNTRY, 'defaultOption'),
        ]}
        isChipInputControl={true}
        chipInputProps={{
          options: entityOptions,
          allowOnlySingleSelect: true,
        }}
        onSearch={sv => loadInitialData()}
        onFiltersChanged={loadInitialData}
      />
      <CustomAgGridReact
        isRowEditing={gridState.isRowEditing}
        rowData={gridState.data}
        gridOptions={gridOptions()}
        serverPagination={true}
        paginationData={gridState.pagination}
        onPaginationChange={request => loadInitialData(request)}
        classes={{ customHeight: classes.customHeight }}
      />
    </>
  );
};

export default inject('sidebarStore', 'permitStore', 'permitSettingsStore')(observer(PermitInfoReview));
