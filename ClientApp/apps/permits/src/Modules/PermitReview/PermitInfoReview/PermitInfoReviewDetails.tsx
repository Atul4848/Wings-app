...
import { permitInfoReviwDetail, fields, isDataMerged, isDataRejected, mergeStatus } from '../fields';
import {
  PermitSettingsStore,
  PermitStore,
...
const PermitInfoReviewDetails: FC<Props> = ({ sidebarStore, permitStore, permitSettingsStore }) => {
  const classes = useStyles();
  const gridState = useGridState();
  const unsubscribe = useUnsubscribe();
  const navigate = useNavigate();
  const params = useParams();
  const agGrid = useAgGrid<'', PermitUplinkStagingReviewModel>([], gridState);
  ...
  useEffect(() => {
    _sidebarStore?.setNavLinks(updatePermitSidebarOptions('Permit Info Review'), 'permits');
  ...
  const loadInitialData = (pageRequest?: IAPIGridRequest) => {
    ...
    _permitStore
      ?.getPermitReviewListDetail(Number(params?.id))
      .pipe(
  ...
  const dialogContent = (data: PermitUplinkStagingReviewModel) => {
    if (!data?.propertyName) return null;

    const matchedField = permitInfoReviwDetail.find(({ match }) => match.test(data.propertyName));
  ...
  const getApprovalsData = (data: PermitUplinkStagingReviewModel) => {
    ModalStore.open(
      <Dialog
  ...
  const isDataDisabled = (data: PermitUplinkStagingReviewModel[]): boolean => {
          ...
          <>
            <ViewPermission hasPermission={permitModuleSecurity.isEditable}>
              <PrimaryButton
                ...
                variant="contained"
                color="primary"
                onClick={() => confirmApprove(Number(params.permitInfoId))}
              >
                Merge All
              </PrimaryButton>
            </ViewPermission>
            <ViewPermission hasPermission={permitModuleSecurity.isEditable}>
              <PrimaryButton
                ...
                variant="contained"
                color="primary"
                onClick={() => openRejectionNotesDialog(VIEW_MODE.EDIT, Number(params.permitInfoId))}
              >
                Reject All
              </PrimaryButton>
            </ViewPermission>
...
export default inject('sidebarStore', 'permitStore', 'permitSettingsStore')(observer(PermitInfoReviewDetails));
