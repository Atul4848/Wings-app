...
import { useStyles } from './CabotageReview.styles';
import {
  BaseCountryStore,
...
const CabotageReview: FC<Props> = ({ operationalRequirementStore, sidebarStore }) => {
  const classes = useStyles();
  const gridState = useGridState();
  const unsubscribe = useUnsubscribe();
  const agGrid = useAgGrid<CABOTAGE_REVIEW_FILTERS, CabotageReviewModel>([], gridState);
  ...
  useEffect(() => {
    _sidebarStore?.setNavLinks(updateCountrySidebarOptions('Cabotage Review'), 'countries');
    loadInitialData();
  }, []);
  ...
  const getFilterPropertyName = (selectedOption): any => {
    switch (selectedOption) {
      case CABOTAGE_REVIEW_FILTERS.COUNTRY:
        return 'countryId';
      case CABOTAGE_REVIEW_FILTERS.APPROVAL_STATUS:
  ...
  const filterCollection = (): IAPIGridRequest => {
    ...
    const _searchValue = chipValue.map(x => {
      return Utilities.isEqual(searchType, CABOTAGE_REVIEW_FILTERS.APPROVAL_STATUS) ? x.value : x.id;
    });
  ...
  const loadInitialData = (pageRequest?: IAPIGridRequest) => {
    ...
    _operationalRequirementStore
      .getCabotageReview(request)
      .pipe(
  ...
  const dialogContent = (data) => {
    const fields = [
      {
        key: 'cabotageEnforcedForFARTypes',
        title: 'Cabotage Enforced for FAR Type',
        formatFn: (item) => `${item?.name}(${item?.cappsCode})` || '',
      },
      {
        key: 'appliedRegionCabotageExemptions',
        title: 'Applied Region Cabotage Exemptions',
        formatFn: (item) => item?.region?.name || '',
      },
      {
        key: 'appliedCountryCabotageExemptions',
        title: 'Applied Country Cabotage Exemptions',
        formatFn: (item) => `${item?.country?.name}(${item?.country?.code})` || '',
      },
    ];
  ...
  const getApprovalsData = data => {
    UIStore.setPageLoader(true);
    _operationalRequirementStore
      ?.getCabotageReviewList(data.cabotageOperationalRequirementStagingId)
      .pipe(
        takeUntil(unsubscribe.destroy$),
        finalize(() => {
  ...
  const ConfirmationModel = (data: CabotageReviewModel, isApprove: boolean): void => {
  ...
  const approveRejectRecord = (data, isApprove: boolean): void => {
    UIStore.setPageLoader(true);
    const request = {
      cabotageOperationalRequirementStagingId: data?.id,
    };
    const operationObservable = isApprove
      ? _operationalRequirementStore?.approveCabotageStaging(request)
      : _operationalRequirementStore?.rejectCabotageStaging(request);
  ...
  const isDataDisabled = data => {
    ...
    if (
      (Utilities.isEqual(data.comparisionType, STAGING_REVIEW_COMPARISION_TYPE.ADDED) &&
        data.vendorCabotageOperationalRequirementId) ||
  ...
  const reviewActions = (rowIndex: number, { data }: RowNode) => {
    ...
    return (
      <UplinkReviewActions
        approveRejectPermission={countryModuleSecurity.isEditable && data.vendorCabotageOperationalRequirementId}
        viewDetailsPermission={countryModuleSecurity.isEditable && data.isList}
  ...
  const gridOptions = (): GridOptions => {
    ...
    return {
      ...
      autoGroupColumnDef: {
        ...
        valueFormatter: ({ data }) => data?.country.label || '',
      },
    };
  };
  ...
  return (
    <>
      <SearchHeaderV3
        ...
        selectInputs={[ agGridUtilities.createSelectOption(CABOTAGE_REVIEW_FILTERS, CABOTAGE_REVIEW_FILTERS.COUNTRY) ]}
...
export default inject('operationalRequirementStore', 'sidebarStore')(observer(CabotageReview));
