...
import { NAME_DESCRIPTION_FILTERS, AircraftOperatorSettings, useRestrictionModuleSecurity } from '../../../Shared';
...
const AircraftOperatorRestrictionType: FC<Props> = ({ ...props }) => {
  ...
  const rightContent = () => {
    ...
    return (
      <PrimaryButton variant="contained" startIcon={<AddIcon />} disabled={gridState.isRowEditing} onClick={addNewType}>
        Add Aircraft Operator Restriction Type
      </PrimaryButton>
    );
  };
  ...
  const loadInitialData = () => {
    ...
    _aircraftOperatorSettingsStore
      .getAircraftOperatorRestrictionTypes()
      .pipe(
        takeUntil(unsubscribe.destroy$),
        finalize(() => UIStore.setPageLoader(false))
      )
      .subscribe(aircraftOperatorRestrictionTypes => gridState.setGridData(aircraftOperatorRestrictionTypes));
  };
  ...
  const upsertAircraftOperatorRestrictionType = (rowIndex: number) => {
    ...
    UIStore.setPageLoader(true);
    _aircraftOperatorSettingsStore
      .upsertAircraftOperatorRestrictionType(model)
      .pipe(
        takeUntil(unsubscribe.destroy$),
        finalize(() => UIStore.setPageLoader(false))
      )
  ...
  const gridActions = (gridAction: GRID_ACTIONS, rowIndex: number) => {
    ...
    switch (gridAction) {
      ...
      case GRID_ACTIONS.SAVE:
        upsertAircraftOperatorRestrictionType(rowIndex);
        break;
...
export default inject('aircraftOperatorSettingsStore')(observer(AircraftOperatorRestrictionType));
