...
import { TraveledHistorySubCategory } from '../Components';
...
describe('Traveled History Subcategory Module', () => {
  ...
  beforeEach(() => {
    wrapper = shallow(<TraveledHistorySubCategory {...props} />).dive();
  ...
  it('should be rendered without errors, should render CustomAgGridReact and SearchHeader ', () => {
    ...
    expect(agGridHelper.getGridOptions().columnDefs?.length).to.be.eq(3);
  });
  ...
  it('should start and stop row editing', () => {
    const upsertSubCategory = sinon.spy();
    ...
    reRender();
    expect(upsertSubCategory.called);
  });

  it('should call upsert function on save button click', () => {
    const mock = sinon.spy(props.settingsStore, 'upsertTraveledHistorySubCategory');
