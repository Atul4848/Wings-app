...
const VendorStatusSettings: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertVendorStatusSettings = (rowIndex: number, agGrid:any, gridState:any): void =>{
    ...
    settingsStore
      ?.upsertSetting(request, apiUrls.vendorStatus, SettingNamesMapper[SETTING_ID.SETTINGS_VENDOR_STATUS])
      .pipe(
  ...
  return (
    <SettingCore 
      ...
      settingsStore={settingsStore}
      settingId={SETTING_ID.SETTINGS_VENDOR_STATUS}
      rightContentAction={false}
      isAuditColumnEnabled={false}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex,agGrid,gridState)=>upsertVendorStatusSettings(rowIndex,agGrid,gridState)} />
  );
};
export default inject('settingsStore')(observer(VendorStatusSettings));
