...
const AddressTypeSettings: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertAddressType = (rowIndex: number, agGrid:any, gridState:any): void =>{
    const model = agGrid._getTableItem(rowIndex)
    const request = new StatusBaseModel({ ...model });
    UIStore.setPageLoader(true);
    gridState.setIsProcessing(true)
    settingsStore
      ?.upsertSetting(request, apiUrls.vendorAddressType, SettingNamesMapper[SETTING_ID.SETTING_ADDRESS_TYPE])
      .pipe(
  ...
  return (
    <SettingCore 
      ...
      collectionName="AddressType"
      rightContentActionText="Add Address Type"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      onSave={(rowIndex,agGrid,gridState)=>upsertAddressType(rowIndex,agGrid,gridState)}
      addressTypeEdit={false} />
  );
};
export default inject('settingsStore')(observer(AddressTypeSettings));
