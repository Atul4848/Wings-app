...
const DocumentStatusSettings: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertDocumentStatus = (rowIndex: number, agGrid:any, gridState:any): void =>{
    const model = agGrid._getTableItem(rowIndex)
    const request = new StatusBaseModel({ ...model });
    UIStore.setPageLoader(true);
    gridState.setIsProcessing(true)
    settingsStore
      ?.upsertSetting(request, apiUrls.vendorDocumentStatus, SettingNamesMapper[SETTING_ID.SETTING_DOCUMENT_STATUS])
      .pipe(
  ...
  return (
    <SettingCore 
      ...
      collectionName="DocumentStatus"
      rightContentActionText="Add Document Status"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      onSave={(rowIndex,agGrid,gridState)=>upsertDocumentStatus(rowIndex,agGrid,gridState)}
    />
  );
};
export default inject('settingsStore')(observer(DocumentStatusSettings));
