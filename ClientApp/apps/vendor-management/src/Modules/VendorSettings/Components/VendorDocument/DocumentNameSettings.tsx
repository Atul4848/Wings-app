...
const DocumentNameSettings: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertDocumentName = (rowIndex: number, agGrid:any, gridState:any): void =>{
    const model = agGrid._getTableItem(rowIndex)
    const request = new StatusBaseModel({ ...model });
    UIStore.setPageLoader(true);
    gridState.setIsProcessing(true)
    settingsStore
      ?.upsertSetting(request, apiUrls.vendorDocumentName, SettingNamesMapper[SETTING_ID.SETTING_DOCUMENT_NAME])
      .pipe(
  ...
  return (
    <SettingCore 
      ...
      rightContentActionText="Add Document Name"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      onSave={(rowIndex,agGrid,gridState)=>upsertDocumentName(rowIndex,agGrid,gridState)}
    />
  );
};
export default inject('settingsStore')(observer(DocumentNameSettings));
