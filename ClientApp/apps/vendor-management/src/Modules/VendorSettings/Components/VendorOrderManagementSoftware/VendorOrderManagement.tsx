...
const VendorOrderManagement: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertFboOne = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        request,
        apiUrls.orderManagementSoftware,
        SettingNamesMapper[SETTING_ID.SETTING_ORDER_MANAGEMENT_SOFTWARE]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Order Management Software"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertFboOne(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(VendorOrderManagement));
