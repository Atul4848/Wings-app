...
const UnitSettings: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertVendorSettingsUnits = (rowIndex: number, agGrid:any, gridState:any): void =>{
    ...
    settingsStore
      ?.upsertSetting(request, apiUrls.vendorSettingsUnits, SettingNamesMapper[SETTING_ID.SETTINGS_UNITS])
      .pipe(
  ...
  return (
    <SettingCore 
      ...
      rightContentActionText="Add Units"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex,agGrid,gridState)=>upsertVendorSettingsUnits(rowIndex,agGrid,gridState)} />
  );
};
export default inject('settingsStore')(observer(UnitSettings))
