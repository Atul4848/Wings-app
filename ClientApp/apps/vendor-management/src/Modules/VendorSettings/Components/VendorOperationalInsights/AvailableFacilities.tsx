...
const AvailableFacilities: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertAvailableFacilities = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        request,
        apiUrls.availableFacilities,
        SettingNamesMapper[SETTING_ID.SETTING_AVAILABLE_FACILITIES]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Available Facilities"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertAvailableFacilities(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(AvailableFacilities));
