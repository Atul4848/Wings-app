...
const TowbarScenarios: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertTowbarScenarios = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        request,
        apiUrls.towbarScenarios,
        SettingNamesMapper[SETTING_ID.SETTING_TOWBAR_SCENARIOS]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Towbar Scenarios"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertTowbarScenarios(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(TowbarScenarios));
