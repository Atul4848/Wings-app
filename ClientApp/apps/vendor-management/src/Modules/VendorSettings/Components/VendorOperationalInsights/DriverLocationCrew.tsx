...
const DriverLocationCrew: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertPickupLocationCrew = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        request,
        apiUrls.pickUpLocation,
        SettingNamesMapper[SETTING_ID.SETTING_DRIVER_LOCATION_CREW]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Pickup Location Crew"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertPickupLocationCrew(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(DriverLocationCrew));
