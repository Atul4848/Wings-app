...
const AircraftParkingOptions: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertAircraftParkingOptions = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        request,
        apiUrls.aircraftParkingOptions,
        SettingNamesMapper[SETTING_ID.SETTING_AIRCRAFT_PARKING_OPTIONS]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Aircraft Parking Option"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertAircraftParkingOptions(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(AircraftParkingOptions));
