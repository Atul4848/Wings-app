...
const DriverLocationPax: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertPickupLocationPax = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        ...
        SettingNamesMapper[SETTING_ID.SETTING_DRIVER_LOCATION_PAX]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Pickup Location Pax"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertPickupLocationPax(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(DriverLocationPax));
