...
const ArrivalCrewPaxPassportHandling: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertArrivalCrewPaxPassportHandling = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        request,
        apiUrls.arrivalCrewPaxPassportHandling,
        SettingNamesMapper[SETTING_ID.SETTING_ARRIVAL_CREW_PAX_PASSPORT_HANDLING]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Arrival Passport Handling"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertArrivalCrewPaxPassportHandling(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(ArrivalCrewPaxPassportHandling));
