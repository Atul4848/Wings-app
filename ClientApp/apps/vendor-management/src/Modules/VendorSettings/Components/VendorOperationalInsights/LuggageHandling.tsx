...
const LuggageHandling: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertLuggageHandling = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        request,
        apiUrls.luggageHandling,
        SettingNamesMapper[SETTING_ID.SETTING_LUGGAGE_HANDLING]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Luggage Handling"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertLuggageHandling(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(LuggageHandling));
