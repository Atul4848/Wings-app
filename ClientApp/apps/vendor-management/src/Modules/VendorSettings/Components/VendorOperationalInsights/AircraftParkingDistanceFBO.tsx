...
const AircraftParkingDistanceFBO: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertAircraftParkingDistanceFBO = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        request,
        apiUrls.aircraftParkingDistanceFBO,
        SettingNamesMapper[SETTING_ID.SETTING_AIRCRAFT_PARKING_DISTANCE_FBO]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Aircraft Parking Distance FBO"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertAircraftParkingDistanceFBO(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(AircraftParkingDistanceFBO));
