...
const Amenities: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertAmenities = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        request,
        apiUrls.amenities,
        SettingNamesMapper[SETTING_ID.SETTING_AMENITIES]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Amenities"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertAmenities(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(Amenities));
