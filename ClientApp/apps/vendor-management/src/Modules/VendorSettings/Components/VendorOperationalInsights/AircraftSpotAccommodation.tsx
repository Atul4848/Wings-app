...
const AircraftSpotAccommodation: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertAircraftSpotAccommodation = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        request,
        apiUrls.aircraftSpotAccommodation,
        SettingNamesMapper[SETTING_ID.SETTING_AIRCRAFT_SPOT_ACCOMMODATION]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Aircraft Spot Accommodation"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertAircraftSpotAccommodation(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(AircraftSpotAccommodation));
