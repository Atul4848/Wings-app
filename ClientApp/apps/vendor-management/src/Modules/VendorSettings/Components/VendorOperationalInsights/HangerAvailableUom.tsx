...
const HangerAvailableUom: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertHangerAvailableUom = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(
        request,
        apiUrls.hangerAvailableUom,
        SettingNamesMapper[SETTING_ID.SETTING_HANGER_AVAILABLE_UOM]
      )
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Hanger Available UOM"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertHangerAvailableUom(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(HangerAvailableUom));
