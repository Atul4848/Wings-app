...
const CommunicationServiceSettings: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertCommunicationService = (rowIndex: number, agGrid:any, gridState:any): void =>{
    ...
    settingsStore
      ?.upsertSetting(
        request, 
        apiUrls.vendorContactCommunicationService, 
        SettingNamesMapper[SETTING_ID.SETTING_COMMUNICATION_SERVICE]
      )
      .pipe(
  ...
  return (
    <SettingCore 
      ...
      collectionName="Communication Service"
      rightContentActionText="Add Communication Service"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      onSave={(rowIndex,agGrid,gridState)=>upsertCommunicationService(rowIndex,agGrid,gridState)} />
  );
};
export default inject('settingsStore')(observer(CommunicationServiceSettings));
