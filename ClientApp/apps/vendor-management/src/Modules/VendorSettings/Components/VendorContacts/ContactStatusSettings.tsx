...
const ContactStatusSettings: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertContactStatus = (rowIndex: number, agGrid:any, gridState:any): void =>{
    const model = agGrid._getTableItem(rowIndex)
    const request = new StatusBaseModel({ ...model });
    UIStore.setPageLoader(true);
    gridState.setIsProcessing(true)
    settingsStore
      ?.upsertSetting(request, apiUrls.vendorContactStatus, SettingNamesMapper[SETTING_ID.SETTINGS_CONTACT_STATUS])
      .pipe(
  ...
  return (
    <SettingCore 
      ...
      collectionName="Contact Status"
      rightContentActionText="Add Status"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      onSave={(rowIndex,agGrid,gridState)=>upsertContactStatus(rowIndex,agGrid,gridState)} />
  );
};
export default inject('settingsStore')(observer(ContactStatusSettings));
