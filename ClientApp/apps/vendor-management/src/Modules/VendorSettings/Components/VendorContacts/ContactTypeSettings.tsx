...
const ContactTypeSettings: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertContactType = (rowIndex: number, agGrid:any, gridState:any): void =>{
    const model = agGrid._getTableItem(rowIndex)
    const request = new StatusBaseModel({ ...model });
    UIStore.setPageLoader(true);
    gridState.setIsProcessing(true)
    settingsStore
      ?.upsertSetting(request, apiUrls.vendorContactType, SettingNamesMapper[SETTING_ID.SETTING_CONTACT_TYPE])
      .pipe(
  ...
  return (
    <SettingCore 
      ...
      collectionName="Contact Type"
      rightContentActionText="Add Contact Type"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      onSave={(rowIndex,agGrid,gridState)=>upsertContactType(rowIndex,agGrid,gridState)} />
  );
};
export default inject('settingsStore')(observer(ContactTypeSettings));
