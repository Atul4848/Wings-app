...
const ParametersSettings: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertVendorSettingsParameters = (rowIndex: number, agGrid:any, gridState:any): void =>{
    ...
    settingsStore
      ?.upsertSetting(request, apiUrls.vendorSettingsParameter, SettingNamesMapper[SETTING_ID.SETTINGS_PARAMETERS])
      .pipe(
  ...
  return (
    <SettingCore 
      ...
      rightContentActionText="Add Parameters"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex,agGrid,gridState)=>upsertVendorSettingsParameters(rowIndex,agGrid,gridState)} />
  );
};
export default inject('settingsStore')(observer(ParametersSettings));
