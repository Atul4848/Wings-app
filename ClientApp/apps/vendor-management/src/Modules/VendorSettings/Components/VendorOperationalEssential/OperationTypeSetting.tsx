...
const OperationTypeSetting: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertOperationType = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(request, apiUrls.operationType, SettingNamesMapper[SETTING_ID.SETTINGS_OPERATON_TYPE])
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Operation Type"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertOperationType(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(OperationTypeSetting));
