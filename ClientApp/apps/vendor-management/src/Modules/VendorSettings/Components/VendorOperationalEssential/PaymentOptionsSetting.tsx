...
const PaymentOptionsSetting: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertPaymentOptions = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(request, apiUrls.paymentOptions, SettingNamesMapper[SETTING_ID.SETTINGS_PAYMENTS_OPTIONS])
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Payment Options"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertPaymentOptions(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(PaymentOptionsSetting));
