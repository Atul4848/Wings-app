...
const VendorLevelSetting: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertVendorLevel = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(request, apiUrls.vendorLevel, SettingNamesMapper[SETTING_ID.SETTINGS_VENDOR_LEVEL])
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Vendor Level"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertVendorLevel(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(VendorLevelSetting));
