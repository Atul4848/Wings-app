...
const CreditAvailableSetting: FC<Props> = ({ settingsStore }) => {
  ...
  const upsertCreditAvailable = (rowIndex: number, agGrid: any, gridState: any): void => {
    ...
    settingsStore
      ?.upsertSetting(request, apiUrls.creditAvailable, SettingNamesMapper[SETTING_ID.SETTINGS_CREDIT_AVAILABLE])
      .pipe(
  ...
  return (
    <SettingCore
      ...
      rightContentActionText="Add Credit Available"
      rightContentAction={VmsModuleSecurity.isEditable}
      isAuditColumnEnabled={true}
      isActionEnabled={true}
      isEditable={VmsModuleSecurity.isEditable}
      selectInputs={[]}
      onSave={(rowIndex, agGrid, gridState) => upsertCreditAvailable(rowIndex, agGrid, gridState)}
    />
  );
};
export default inject('settingsStore')(observer(CreditAvailableSetting));
