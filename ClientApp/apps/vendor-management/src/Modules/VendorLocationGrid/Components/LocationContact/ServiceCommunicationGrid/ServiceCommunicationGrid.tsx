...
import { AuditHistory, baseApiPath, useBaseUpsertComponent } from '@wings/shared';
...
const ServiceCommunicationGrid: FC<Props> = ({
    ...
    const baseOptions: Partial<GridOptions> = agGrid.gridOptionsBase({
      ...
      gridActionProps: {
        ...
        onAction: (action: GRID_ACTIONS, rowIndex: number) => {
          const model: ServiceCommunicationPreferenceModel = agGrid._getTableItem(rowIndex);
          switch (action) {
            ...
            case GRID_ACTIONS.AUDIT:
              ModalStore.open(
                <AuditHistory
                  title={model.name}
                  entityId={model.id}
                  entityType={VENDOR_AUDIT_MODULES.VENDORLOCATIONSERVICECOMM}
                  baseUrl={`${baseApiPath.vendorManagementCoreUrl}`}
                  schemaName={VENDOR_AUDIT_MODULES.VENDORLOCATIONSERVICECOMM}
  ...
  const cancelEditing = (rowIndex: number) => {
    ...
    gridState.setIsAllRowsSelected(false);
    setServiceCommGridDisabled(false);
    gridState.hasError ? loadVendorServiceCommData() : ''
  };
