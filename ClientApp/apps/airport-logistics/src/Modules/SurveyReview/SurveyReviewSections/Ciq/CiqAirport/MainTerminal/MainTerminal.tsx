...
import { OperatingHoursModel, MainTerminalModel } from './../../../../../Shared/Models/index';
import { OperatingHours } from './../../../../OperatingHours/OperatingHours';
import MainTerminalEditor from './MainTerminalEditor';
import { SurveyReviewLabel } from '../../../SurveyReviewSection';
import { CIQ_LOGISTICS_CREW_PAX_AVAILABLE_FACILITY } from './../../../../../Shared/Enums/index';
...
type Props = {
  approved?: MainTerminalModel;
  unApproved?: MainTerminalModel;
  isEditMode?: boolean;
  field: Field;
  classes?: IClasses;
};
...
@observer
class MainTerminal extends Component<Props> {
  private get editor(): ReactNode {
    if (!this.props.isEditMode) {
      return null;
    }

    return <MainTerminalEditor field={this.props.field} />;
  }
  ...
  private getDetails(item: MainTerminalModel): ReactNode {
    ...
    return (
      <div className={classes.hourDetails}>
        <div className={classes.hourDetailsRow}>
          ...
          <div className={classes.hourDetailValue}>{item.ciqRequired || emptyLabel}</div>
        </div>
        <div className={classes.hourDetailsRow}>
          <div className={classes.hourDetailLabel}>
            {CIQ_LOGISTICS_CREW_PAX_AVAILABLE_FACILITY.CREW_PAX_PRIORITY}
          </div>
          <div className={classes.hourDetailValue}>{item?.crewPaxPriority || emptyLabel}</div>
        </div>
        <div className={classes.hourDetailsRow}>
          <div className={classes.hourDetailLabel}>
            {CIQ_LOGISTICS_CREW_PAX_AVAILABLE_FACILITY.CREW_PAX_PRIORITY_EXPLANATION}
          </div>
          <div className={classes.hourDetailValue}>{item?.crewPaxPriorityExplanation || emptyLabel}</div>
        </div>
      </div>
    );
  }
...
export default withStyles(styles)(MainTerminal);
