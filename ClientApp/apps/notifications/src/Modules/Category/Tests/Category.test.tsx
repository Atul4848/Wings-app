...
import { CategoryModel, CategoryStoreMock, CATEGORY_FILTERS } from '../../Shared';
import { PureCategory } from '../Category';
...
describe('Category Module', () => {
  ...
  let instance;
  const categoryStore = new CategoryStoreMock();
  ...
  beforeEach(() => {
    gridApi = new GridApiMock({ data: categoryData });
    wrapper = shallow(
      <div>
        <PureCategory categoryStore={categoryStore} />
        <ModalKeeper />
      </div>
    );
    modal = wrapper
  ...
  it('SearchHeader sets searchValue and it calls gridApi', () => {
    ...
    searchHeader().simulate('search', 'Maintenance_Notifications');
    expect(instance.searchValue).to.equal('Maintenance_Notifications');
    expect(instance.gridApi.onFilterChanged.calledOnce).to.be.true;
  });

  it('SearchHeader sets selectedOption and it calls gridApi', () => {
    instance.gridApi = gridApi;
    searchHeader().simulate('searchTypeChange', CATEGORY_FILTERS.NAME);
    expect(instance.selectedOption).to.equal(CATEGORY_FILTERS.NAME);
  ...
  it('Add button should call Grid API and hasError to true', () => {
    ...
    expect(primaryButton().text()).to.equal('Add Category');
    instance.gridApi = gridApi;

    // should start editing cell if new row added
    const addSpy = sinon.spy(instance, 'addCategory');
    primaryButton().simulate('click');
    expect(instance.hasError).to.true;
    expect(addSpy.calledOnce).to.be.true;
  });

  it('should check if Category already exists', () => {
    instance.data = [categoryData];

    //should check for duplicate category name
    instance.gridApi = {
      ...new GridApiMock(),
      getCellEditorInstances: () => [{ getValue: () => 'Maintenance_Notifications' }],
    };
  ...
  it('should Grid action perform cases and default on Stop Editing', () => {
    const model = new CategoryModel({ id: 0 });
  ...
  it('should close dialog with confirm dialog no Click', () => {
    ...
    instance.confirmRemoveCategory(1);
  ...
  it('should close dialog with confirm dialog yes Click', () => {
    instance.gridApi = gridApi;
    const removeCategory = sinon.spy(instance, 'removeCategory');
    instance.confirmRemoveCategory(1);
    modal.find(ConfirmDialog).simulate('yesClick');
    expect(removeCategory.called).to.be.true;
  });

  it('should check if Category name already does not exists', () => {
    let cells = [
      {
        getFrameworkComponentInstance: () => ({ hasError: false }),
        getValue: () => categoryData,
      },
    ];
    instance.data = [new CategoryModel({ id: 2, name: 'Trips_Notifications' })];
