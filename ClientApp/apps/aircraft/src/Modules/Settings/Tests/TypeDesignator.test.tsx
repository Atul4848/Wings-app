...
import { TypeDesignator } from '../Components';

describe('TypeDesignator Module', () => {
  ...
  beforeEach(() => {
    wrapper = shallow(<TypeDesignator {...props} />).dive();
  ...
  it('should call upsert function on save button click', () => {
    const mock = sinon.spy(props.settingsStore, 'upsertICAOTypeDesignator');
  ...
  it('onDropDownChange should set hasError value based on changes', () => {
    const parentComp = agGridHelper.getParentComponent();
    parentComp.onDropDownChange({ colDef: { field: 'propulsionType' } } as any, '');
  });
