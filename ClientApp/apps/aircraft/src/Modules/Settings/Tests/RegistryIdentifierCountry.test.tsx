...
import RegistryIdentifierCountry from '../Components/RegistryIdentifierCountry/RegistryIdentifierCountry';
import { SearchHeaderV3 } from '@wings-shared/form-controls';

describe('RegistryIdentifierCountry module', () => {
  ...
  beforeEach(() => {
    wrapper = shallow(<RegistryIdentifierCountry {...props} />).dive();
  ...
  it('should call upsert function on save button click', () => {
    const mock = sinon.spy(props.settingsStore, 'upsertRegistryIdentifierCountry');
  ...
  it('onDropDownChange should set hasError value based on changes', () => {
    ...
    parentComp.onDropDownChange({ colDef: { field: 'country' } } as any, '');
  });

  it('should change the values with onInputChange function', () => {
    const parentComp = agGridHelper.getParentComponent();
    parentComp.onInputChange({ colDef: { field: 'Code' } } as any, '');
  });
});
