...
import { FlightPlanFormatChangeRecordModel } from '../../Shared';
import { ModalStore } from '@uvgo-shared/modal-keeper';
import { CustomAgGridReact } from '@wings-shared/custom-ag-grid';
import { FlightPlanChangeRecordGrid } from '../Component';
import { GRID_ACTIONS } from '@wings-shared/core';
import { CollapsibleWithButton } from '@wings-shared/layout';

describe('FlightPlanChangeRecordGrid module', () => {
  let wrapper: any;
  let agGridHelper: AgGridTestingHelper;

  const props = {
    isEditable: true,
    flightPlanFormatChangeRecords: [new FlightPlanFormatChangeRecordModel()],
    onDataSave: sinon.fake(),
    onRowEditing: sinon.fake(),
  };

  beforeEach(() => {
    wrapper = shallow(<FlightPlanChangeRecordGrid {...props} />);
  ...
  it('should add 5 columns in the table', () => {
    expect(wrapper.find(CustomAgGridReact).props().gridOptions.columnDefs).to.have.length(5);
  });
  ...
  it('should change the values with onInputChange function', () => {
    ...
    parentComp.onInputChange({ colDef: { field: 'requestedBy' } } as any, '');
  });
