...
import { EtpScenarioModel, EtpScenarioStoreMock, EtpSettingsStoreMock, SettingsStoreMock } from '../../Shared';
import { SearchHeaderV3 } from '@wings-shared/form-controls';
import { SidebarStore } from '@wings-shared/layout';
import EtpScenario from '../EtpScenario';

describe('ETP Scenario Module', () => {
  let wrapper: ShallowWrapper;
  let agGridHelper: AgGridTestingHelper;
  let etpScenarioStore = new EtpScenarioStoreMock();
  let etpSettingsStore = new EtpSettingsStoreMock();
  let settingsStore = new SettingsStoreMock();
  const etpScenario = new EtpScenarioModel({
    id: 0,
  });

  const props = { etpScenarioStore, etpSettingsStore, settingsStore, sidebarStore:SidebarStore };
  const reRender = () => wrapper.setProps({ abc: Math.random() });

  beforeEach(() => {
    wrapper = shallow(<EtpScenario {...props} />).dive();
