...
import { FuelStoreMock } from '../../Shared';
import { ViewInputControl } from '@wings-shared/form-controls';

describe('Upsert Fuel Dialog', () => {
  ...
  const props = {
    ...
    upsertFuel: sinon.fake(),
    viewMode: VIEW_MODE.EDIT,
    uwaCustomerId: 1,
  };

  const renderView = props => {
    wrapper = shallow(<UpsertFuel {...props} />).dive().shallow();
  ...
  it('save button should call UpsertFuel on props', () => {
    dialogContent.find(PrimaryButton).simulate('click');
    expect(props.upsertFuel.calledOnce).to.true;
  });
  ...
  it('onValueChange should update form value', () => {
    ...
    instance.getField('uwaCustomerId').set = mockFormUpdate;
    instance.onValueChange('test', 'uwaCustomerId');
