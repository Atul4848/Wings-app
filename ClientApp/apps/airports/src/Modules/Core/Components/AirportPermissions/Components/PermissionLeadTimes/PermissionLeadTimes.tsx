...
import { AirportSettingsStore, PermissionLeadTimeModel, useAirportModuleSecurity } from '../../../../../Shared';
import { ColDef, GridOptions, ValueFormatterParams, ICellEditorParams, GridReadyEvent } from 'ag-grid-community';

interface Props {
  airportSettingsStore?: AirportSettingsStore;
  isEditable: boolean;
  leadTimes: PermissionLeadTimeModel[];
  onGridDataUpdate: (gridName: string, gridData: PermissionLeadTimeModel[]) => void;
  isRowEditing: (isEditing: boolean) => void;
}

const PermissionLeadTimes: FC<Props> = ({ ...props }) => {
  const gridState = useGridState();
  const classes = useStyles();
  const agGrid = useAgGrid<'', PermissionLeadTimeModel>([], gridState);
  ...
  const addLeadTime = (): void => {
    ...
    agGrid.addNewItems([ new PermissionLeadTimeModel({ id: 0 }) ], {
      startEditing: false,
      colKey: 'leadTimeType',
    });
  ...
  const updateTableData = (): void => {
    ...
    props.onGridDataUpdate('permissionLeadTimes', gridState.data);
  ...
  const columnDefs: ColDef[] = [
    {
      ...
      cellEditorParams: {
        ...
        getAutoCompleteOptions: () => _airportSettingStore.permissionLeadTimeTypes,
      },
    },
    {
    ...
    {
      headerName: 'Lead Time UOM',
      field: 'leadTimeUOM',
      cellEditor: 'customAutoComplete',
      valueFormatter: ({ value }) => value?.name || '',
      cellEditorParams: {
        isRequired: true,
        placeHolder: 'Select Lead Time UOM',
        getAutoCompleteOptions: () => _airportSettingStore.leadTimeUOMs,
      },
    },
    {
      ...agGrid.actionColumn({
  ...
  const gridOptions = (): GridOptions => {
    ...
    return {
      ...
      onRowEditingStarted: params => {
        ...
        _airportSettingStore?.loadPermissionLeadTimeTypes().subscribe();
        _airportSettingStore?.loadLeadTimeUoms().subscribe();
      },
    };
  };
...
export default inject('airportSettingsStore')(observer(PermissionLeadTimes));
