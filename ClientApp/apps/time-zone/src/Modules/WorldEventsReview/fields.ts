...
import {  WorldEventsReviewModel } from '../Shared';
...
export const getGridData = (
  ...
  const tableData = data.reduce((total, item, index) => {
    ...
    total.push(WorldEventsReviewModel.deserialize({ ...item, uplinkStagingProperties: [] }));
    // Check if child available
    if (item.uplinkStagingProperties?.length) {
      const result = getGridData(item.uplinkStagingProperties, item.path, true, item.worldEventStagingId);
