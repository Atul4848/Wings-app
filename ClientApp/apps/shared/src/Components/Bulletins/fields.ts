...
import { ASSOCIATED_BULLETIN_FILTERS, BULLETIN_FILTERS } from './Enums';
...
export const gridFilters: IAPIFilterDictionary<BULLETIN_FILTERS>[] = [
  ...
  {
    columnId: 'vendorLocationAirport',
    apiPropertyName: 'VendorLocationAirport.DisplayCode',
    uiFilterType: BULLETIN_FILTERS.VENDOR_LOCATION_AIRPORT_CODE,
  },
  {
...
export const associatedBulletinFilters: IAPIFilterDictionary<ASSOCIATED_BULLETIN_FILTERS>[] = gridFilters
  .filter(filter => ![ BULLETIN_FILTERS.BULLETIN_ENTITY ].includes(filter.uiFilterType))
  .map(filter => ({
    ...filter,
    uiFilterType: (filter.uiFilterType as unknown) as ASSOCIATED_BULLETIN_FILTERS,
  }));
